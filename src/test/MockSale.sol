...
import {ISale} from "src/Sale.sol";
...
contract MockSale is ISale, ERC165 {
    ...
    function paymentTokenToToken(uint256 _x) external pure override(ISale) returns (uint256) {
        return _x;
    }

    function tokenToPaymentToken(uint256 _x) external pure override(ISale) returns (uint256) {
        return _x;
    }
    ...
    function uncappedAllocation(address to) external view override(ISale) returns (uint256) {
    ...
    function test_Called(string calldata name, uint256 amount) external view returns (bool) {
